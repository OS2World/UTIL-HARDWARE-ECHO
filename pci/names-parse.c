...
void 
pci_set_name_list_mem_buf (struct pci_access *a, char *buf, int to_be_freed) PCI_ABI
{
  if (a->free_id_mem_buf)
  {
    free (a->id_mem_buf);
  }
  a->id_mem_buf = buf;
  a->free_id_mem_buf = to_be_freed;
}
